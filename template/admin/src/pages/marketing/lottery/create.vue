        ...
        <el-col :span="24" v-loading="spinShow">
          ...
          >
            <el-row>
              ...
              >
                <el-form-item
                  :label="formValidate.factor == 1 ? '抽奖消耗权益值：' : '抽奖次数：'"
                  :prop="
...
export default {
  ...
  data() {
    return {
      ...
      specsData: [
        {
          type: 1, //类型 1：未中奖 2：权益值  3:余额  4：红包 5:优惠券 6：站内商品
          name: '', //活动名称
          num: 10, //奖品数量
          image: '', //奖品图片
          chance: 1, //中奖权重
      ...
      formValidate: {
        ...
        desc: '', //活动描述
        image: '', //活动背景图
        factor: '1', //抽奖类型：1:权益值 2:余额 3：下单支付成功 4:订单评价',5:关注
        ...
        .then((res) => {
          if (res.status == 200 && !Array.isArray(res.data)) {
            ...
            this.formValidate = {
              ...
              desc: '', //活动描述
              image: '', //活动背景图
              factor: e.toString(), //抽奖类型：1:权益值 2:余额 3：下单支付成功 4:订单评价',5:关注
              factor_num: 1, //获取一次抽奖的条件数量
              attends_user: 1, //参与用户1：所有  2：部分
              user_level: [], //参与用户等级
              user_label: [], //参与用户标签
